...
namespace Generator
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var procedures = new Handler(connectionString).GetProcedures("app", "i18n", "auth", "dbo");
            ...
            code.AppendLine("     {");
            code.AppendLine("         public DbProcedureService(AspNetCore.Lib.Configurations.IAppSettings appSettings)");
            code.AppendLine(
        ...
        static void GenerateInterfaceFile(IEnumerable<StoredProcedure> procedures)
        {
            ...
            var path = Path.Combine(Directory.GetParent(CurrentDirectory).FullName, "DataAccess/IDbProcedureService.cs");
            File.WriteAllText(path, code.ToString());
        }
